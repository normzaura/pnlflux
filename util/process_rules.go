...
// tintBlueLastMonth applies a blue fill to the last month cell of the row.
// Used for matched rows where no k is set and threshold cannot be computed.
func tintBlueLastMonth(f *excelize.File, sheet string, rowNum int, cells []string, monthCols []int, styleCache map[int]int) error {
	if len(monthCols) == 0 {
		return nil
	}
	lastCol := monthCols[len(monthCols)-1]
	...
	if !ok {
		...
		merged, err := f.NewStyle(&excelize.Style{
			...
			NumFmt:       existing.NumFmt,
			CustomNumFmt: existing.CustomNumFmt,
			Fill:         excelize.Fill{Type: "pattern", Color: []string{"#0070C0"}, Pattern: 1},
		})
		if err != nil {
			return err
		}
